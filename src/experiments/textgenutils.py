"""Utils for generating text from a language model (Jupyter-oriented)."""
...
from src.nlp.generation import batch_generation, top_k_sample, nucleus_sample  # noqa: F401 (re-exported for notebooks)
...
def generate_text(model,
                  ...
                  use_kv_cache: bool = True,
                  stdout = None):
    r"""Given a conditioning prompt, generates N tokens.
    ...
    indices = torch.tensor([tokenizer.encode(prompt)], device=device, dtype=torch.long)
    
    generator = batch_generation(model, indices,
