...
class GroupedQueryRotaryAttention(Module):
    ...
    def __init__(self,
        ...
        self.W_k = Linear(embed_dim, n_kv_heads * self.per_head_dim, bias=bias)
    ...
    def forward(self,
        ...
        def reshape_and_transpose(tensor, n_heads):
            """Reshapes tensor with shape (batch, seqlen, n_heads * per_head_dim)
                                 to shape (batch, n_heads, seqlen, per_head_dim)."""
            (batch, seqlen, _) = tensor.shape
            tensor = tensor.reshape((batch, seqlen, n_heads, self.per_head_dim))
    ...
    def compute_rotation_matrix(self):
        ...
        angle = np.outer(
            ...
            1.0 / self.rotary_base ** (2 * np.arange(self.per_head_dim // 2) / self.per_head_dim)
        )
    ...
    def apply_rotation_matrix(self, qk: Tensor, rotation_matr: Tuple[Tensor, Tensor], offset: int):
        """Rotates qk = shape (batch, n_heads, seqlen, per_head_dim) by `rotation_matr`.
        
        For each 2D point (x, y) at seqlen index `i` and per_head_dim indices `(2j, 2j+1)`,
        rotate (x, y) by angle A := i / rotary_base^(2j / per_head_dim).
        ...
        # Reshape to (batch, head, seqlen, per_head_dim/2, 2)
        ...
        # Reshape back to (batch, head, seqlen, per_head_dim)
...
class MultiheadAttention(Module):
    ...
    def __init__(self,
        ...
        # kv_cache == (key_cache, value_cache), both of shape (batch, n_heads, cache_seqlen, per_head_dim).
    ...
    def forward(self,
        ...
        def reshape_and_transpose(tensor):
            """Reshapes tensor with shape (batch, seqlen, n_heads * per_head_dim)
                                 to shape (batch, n_heads, seqlen, per_head_dim)."""
            (batch, seqlen, _) = tensor.shape
            tensor = tensor.reshape((batch, seqlen, self.n_heads, self.per_head_dim))
            tensor = tensor.transpose(1, 2)
            return tensor
