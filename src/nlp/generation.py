...
def _get_generation_logits(model,
                           ...
                           use_kv_cache: bool):
    """model(...) may return logits or (logits, loss); return logits."""
...
def batch_generation(model, 
    ...
    for token_n in range(n_tokens_to_gen):
        ...
        try:
            ...
            if use_kv_cache:
                _ = _get_generation_logits(model, next_indices_tensor, use_kv_cache=True)
            return
...
def top_k_sample(probs: torch.Tensor,
    ...
    top_k_prob = torch.sort(probs, dim=0)[0][-min(top_k, probs.shape[0]), :]
...
def default_modify_kv_cache(model,
    ...
    """Trim each layer's KV cache to `trim_seqlen` positions and/or select beams (batch rows).

    kv_cache entries are (batch, n_kv_heads, cache_seqlen, dim_head).
    """
    for decoder_block in model.decoder_blocks:
        if decoder_block.attn.kv_cache is None:
            continue
        (key_cache, value_cache) = decoder_block.attn.kv_cache

        if trim_seqlen is not None:
            if trim_seqlen <= 0:
                decoder_block.attn.kv_cache = None
                continue
            key_cache = key_cache[:, :, :trim_seqlen]
            value_cache = value_cache[:, :, :trim_seqlen]
        if reindex_kv_indices is not None:
